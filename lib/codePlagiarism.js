...
export function analyzeCodeEntropy(code) {
    ...
    if (tokenEntropy >= 3.5 && tokenEntropy <= 5.0 && totalWords > 30) {
        aiIndicatorScore += 15;
    ...
    if (uniqueTokenRatio < 0.35 && totalWords > 20) {
        aiIndicatorScore += 20;
        issues.push(`Low vocabulary diversity: ${(uniqueTokenRatio * 100).toFixed(0)}% unique tokens`);
    } else if (uniqueTokenRatio < 0.50 && totalWords > 20) {
        aiIndicatorScore += 10;
...
export function analyzeHalsteadComplexity(code) {
    ...
    // AI code has moderate difficulty (5-20) — not too simple, not too complex
    if (difficulty >= 5 && difficulty <= 25 && length > 30) {
        aiIndicatorScore += 10;
    ...
    if (estimatedBugs < 0.5 && length > 50) {
        aiIndicatorScore += 12;
...
export function analyzeCodeNaturalness(code) {
    ...
    if (!mixedSemicolons && nonEmptyLines.length > 10) {
        naturalScore -= 8;
    ...
    if (trailingWhitespace === 0 && lines.length > 5) {
        naturalScore -= 5;
    ...
    if (!hasDebugCode && nonEmptyLines.length > 15) {
        naturalScore -= 5;
    ...
    if (commentedOutCode === 0 && nonEmptyLines.length > 15) {
        naturalScore -= 5;
...
export async function analyzeCodePlagiarism(code, language = 'auto', genAI = null, compareWith = null) {
    ...
    // --- AI detection penalty (combined from all engines) ---
    if (results.aiGeneratedPercentage > 15) {
    ...
    if (penalties.length > 0) {
        ...
        // Use higher of: weighted average or 80% of max penalty
        const finalPenalty = Math.max(weightedPenalty, maxPenalty * 0.80);
